...
class BallsPuzzleActionButtonComponent extends StatefulWidget {
  const BallsPuzzleActionButtonComponent({
  ...
  @override
  State<BallsPuzzleActionButtonComponent> createState() =>
      _BallsPuzzleActionButtonComponentState();
}

class _BallsPuzzleActionButtonComponentState
    extends State<BallsPuzzleActionButtonComponent> {
    ...
    return AudioControlListener(
      ...
      child: AnimatedSwitcher(
        ...
        child: BlocBuilder<BallsPuzzleBloc, BallsPuzzleState>(
          builder: (BuildContext context,BallsPuzzleState state) {
            return Tooltip(
              key: ValueKey(state.status),
              message: state.status == BallsPuzzleStatus.started ? 
              'Restarting the puzzle will reset your score':'',
              verticalOffset: 40,
              child: PuzzleButton(
                onPressed:  state.status == BallsPuzzleStatus.loading
                ? null
                : () async {
                    // Reset the timer and the countdown.
                    context.read<TimerBloc>().add(const TimerReset());
                    context.read<BallsPuzzleBloc>().add(
                      BallsCountdownReset(
                        secondsToBegin:state.status==BallsPuzzleStatus.started
                        ? 5 : 3,
                      ),
                    );
                ...
                textColor: state.status == BallsPuzzleStatus.loading
                ? Theme.of(context).colorScheme.onPrimary 
                : Theme.of(context).colorScheme.primary,
                child: state.status == BallsPuzzleStatus.started 
                ? Text('Restart',
                  style: TextStyle(
                ...
                : Text(state.status == BallsPuzzleStatus.loading
